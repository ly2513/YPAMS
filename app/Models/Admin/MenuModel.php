...
class MenuModel extends \YP\Core\YP_Model
{
    ...
    public static function filterMapList($data, $status)
    {
        ...
        $finalData['icon']      = $data['icon'];
        $status1               = ($data['url'] == '#') || !$data['url'];
        if (!$status1) {
            $url            = array_filter(explode('/', $data['url']));
            $num            = count($url);
            $finalData['m'] = $num == 3 ? $url[1] : '';
            $finalData['c'] = $num == 3 ? $url[2] : $url[1];
            $finalData['a'] = $num == 3 ? $url[3] : $url[2];
        }
        if (!$status) {
